...
namespace PS2::Sly1
{
	...
	void Freecam::update()
	{
		...
		if (m_isEnabled)
		{
			...
			m_camera.rotateX(m_controls.rollVelocity(Input::RotateXPos, Input::RotateXNeg));
			m_camera.rotateY(m_controls.pitchVelocity(Input::RotateYPos, Input::RotateYNeg));
			m_camera.rotateZ(m_controls.yawVelocity(Input::RotateZPos, Input::RotateZNeg));
