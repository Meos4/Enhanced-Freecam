...
namespace PS2::Sly1
{
	static constexpr Input3D input3D
	{
		...
		.up = Input::MoveUp,
		.down = Input::MoveDown,
		.rotateRollPos = Input::RotateXPos,
		.rotateRollNeg = Input::RotateXNeg,
		.rotatePitchPos = Input::RotateYPos,
		.rotatePitchNeg = Input::RotateYNeg,
		.rotateYawPos = Input::RotateZPos,
		.rotateYawNeg = Input::RotateZNeg,
		.fovPos = Input::FovPos,
		.fovNeg = Input::FovNeg
	};
	...
	void Freecam::update()
	{
		...
		if (m_isEnabled)
		{
			...
			m_camera.rotateX(m_controls.rollVelocity(dt));
			m_camera.rotateY(m_controls.pitchVelocity(dt));
			m_camera.rotateZ(m_controls.yawVelocity(dt));
