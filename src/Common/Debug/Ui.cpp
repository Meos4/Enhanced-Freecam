...
namespace Debug::Ui
{
	void mipsCallWindow(const Ram& ram, bool* isOpen)
	{
		...
		::Ui::drag("Begin", &data.begin, u32(4), "%X", 0, u32(0), ramSize);
		data.begin = std::clamp(data.begin - (data.begin % 4), u32(0), ramSize);
		ImGui::PopItemWidth();

		ImGui::SameLine();
		::Ui::drag("Size", &data.size, u32(4), "%X", 0, u32(0), ramSize);
		...
		if (data.call.size())
		{
			...
			for (std::size_t i{}; i < data.call.size(); ++i)
			{
				auto* const c{ &data.call[i] };
				...
				if (::Ui::button(formattedOffset.c_str()))
				{
